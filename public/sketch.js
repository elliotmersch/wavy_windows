...
function draw() {
  let p = colorOptions[ceil(random(0, colorOptions.length)) - 1];
  console.log("p = " + p);
  palette = p;
  background(random(palette));
  let w = random(30, 150); //width of each line
  console.log("w = " + ceil(w) + "  [30, 150]");
  let s = random(5, 40); //space b/w each indv row
  console.log("s = " + ceil(s) + "  [5, 40]");
  let i = random(2, 9); //wave intensity
  console.log("i = " + ceil(i) + "  [20, 90]");
  let r = random(-35, 35); //random rotation
  console.log("r = " + ceil(r) + "  [-35, 35]");
  push();
    translate(width, -height);
    rotate(r);
    lineGroup(w, s, i, palette);
  pop();
  drawFrame();
}

function lineGroup(w, s, i, palette) {
  push();
  //row loop
  for (var e = -height*2; e <= height*2; e += s){
    var f = random(palette);
    while(f == prevF){
      f = random(palette);
    }
    fill(f);
    ...
    beginShape();
      ...
      //draw wave line for the current row. randomize width of each line
      for(var x = -width*2; x <= (width*2); x += random(w/2, w)){
        y = (noise(x+e)*i);
        curveVertex(x, y);
        //circle(x, y, w * random(1, 3));
      }

      //complete the bottom right and bottom left of box to fill
      vertex(width*2, y);
      vertex(width*2, height*2);
      vertex(-width*2, y/2);
      //curveVertex(-width*2, y/2);
    endShape(CLOSE);
...
function drawFrame() {
  let s = random(0, 1);
  fill(255);
  //var fW = random(10, 20); //frame width
  var fW = 15;

  if (s < 0.33){
    rect(0, 0, width, height/fW); //top
    rect(0, 0, width/fW, height); //left
    rect(width-(width/fW), 0, width/fW, height); //right
    rect(0, height-(height/fW), width, height/fW); //bottom
    rect(width/2-((width/fW)/2), 0, width/fW, height); //2nd middle
    rect(0, height/2-(height/fW/2), width, height/fW); //h middle
    if(random(0,1) > 0.5){
          rect(width/4-(width/fW)/4, 0, width/fW, height); //1st middle
          rect((width/4-(width/fW)/4)*3, 0, width/fW, height); //3rd middle
    }
  } else if (s >= 0.33 && s < 0.66){
    beginShape();
      vertex(0, 0);
      vertex(width, 0);
      vertex(width, height);
      ...
      beginContour();
        vertex(fW*5, fW*5);
        vertex(fW*5, height - fW*5);
        vertex(width - fW*5, height - fW*5);
        vertex(width - fW*5, fW*5);
      endContour();
    endShape();
    if (random(0, 1) < 0.5){
      rect(0, height/2-(height/fW/2), width, height/fW); //h middle
    ...
    beginShape();
      ...
      beginContour();
        vertex(width / 2, fW * 5);
        quadraticVertex(fW * 5, fW * 5, fW * 5, height / 2);
        quadraticVertex(fW * 5, height - fW * 5, width / 2, height - fW * 5);
        quadraticVertex(width - fW * 5, height - fW * 5, width - fW * 5, height / 2);
        quadraticVertex(width - fW * 5, fW * 5, width / 2, fW * 5);
      endContour();
    endShape();
  }
}
